...
export function formatMissingRipgrepMessage(result: ToolUnavailableResult): string {
	...
	switch (result.reason) {
		...
		case "manual_install_required":
			reason = "AMIDE cannot install this helper automatically in Termux.";
			break;
		...
		case "download_failed": {
			...
			reason = detail
				? `AMIDE could not install it automatically: ${detail}`
				: "AMIDE could not install it automatically.";
			break;
		}
	}

	return [
		"ripgrep (rg) is an optional search helper. Without it, model-run file searches may be slower or fail; AMIDE and subagents remain available.",
		reason,
