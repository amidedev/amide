...
```
~/.amide/agent/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.amide/agent/sessions/`.

AMIDE also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, AMIDE uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from AMIDE AI)
...
### Extended Message Types (from AMIDE Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by AMIDE (legacy field name)
