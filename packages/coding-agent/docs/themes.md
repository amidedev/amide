> AMIDE can create themes. Ask it to build one for your setup.
...
AMIDE loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.amide/agent/themes/*.json`
- Project: `.amide/agent/themes/*.json`
...
On first run, AMIDE detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.amide/agent/themes
vim ~/.amide/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, AMIDE reloads it automatically for immediate visual feedback.
...
AMIDE uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, AMIDE falls back to the nearest approximation.
