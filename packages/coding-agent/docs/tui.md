> AMIDE can create TUI components. Ask it to build one for your use case.
...
AMIDE shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while AMIDE is streaming a response.
...
// Restore AMIDE's default spinner
ctx.ui.setWorkingIndicator();
```
